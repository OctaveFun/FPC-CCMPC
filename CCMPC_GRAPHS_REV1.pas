...
Begin
 ...
 start      := 40 ;                    {reason for 30 is because half of log files have jibberish data at the beginning}
 finish     := (LogLength-80);         {-80 needed to prevent range error from mouse position, bug I gave up finding}
 ...
 setTextStyle(1,0,1);
   repeat   {graph re-plotted at each graph control input}
    ...
    Begin
      ...
      MoveTo(mposX1, 20); LineTo(mposX1, (MaxY-BottomBorder));      {drawing white tracer line}
        ...
        repeat                                 {***graph controls input here***}
        ...
        if lpressed Then
           Begin
             ...
             Else
               Begin
                 for k := Low(ValueAtMousePos) to High(ValueAtMousePos) Do
                 ValueAtMousePos[k] := channels[k,(start + round((mposX1-20)/step))] ;
               End;
        End; {if lpressed}
        if rpressed Then Begin start:=30; finish:=(LogLength-80); end;
        ...
        if keypressed then
           Begin
            ...
            #115  : begin
                      ...
                      if (finish < (LogLength - 120)) then finish := finish + 40;
                    end;
